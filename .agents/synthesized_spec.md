# Synthesized Specification V5.0 (Semantic Domain Graph & Compiler)

**Intent**: Implement features for roles: ['instructor', 'driving school management', 'student']. Archetypes: ['fullstack', 'mobile_hybrid']
**Archetypes**: fullstack, mobile_hybrid | **Scope**: MAJOR
**Gate Result**: PASS_WITH_DECISIONS (Assumption Weight: 50/150)
**Domain Specificity Score**: 75% | **Unsupported Invention Rate**: 0%
**Total Requirements**: 348 | **Version**: v8

## ❓ Questions for Human Review (Dependency Holes & Clarifications)
...
## 🕸️ Semantic Domain Graph Topology

- **Total Domain Nodes**: 7
- **Total Relational Edges**: 6

| Primitive Type | Node Name | Provenance | ID |
|---|---|---|---|
| `actor` | **Operator** | `explicit` | `actor_operator` |
| `entity` | **School** | `explicit` | `entity_school` |
| `entity` | **Instructor** | `explicit` | `entity_instructor` |
| `entity` | **Portal** | `explicit` | `entity_portal` |
| `entity` | **Exam** | `explicit` | `entity_exam` |
| `entity` | **Student** | `explicit` | `entity_student` |
| `entity` | **Enrollment** | `explicit` | `entity_enrollment` |

## 🗺️ Role-Based Page Spreads & Frontend Sitemap

### Role: OPERATOR (8 Pages)

| Route Path | Page Name | Module Scope | Description |
|---|---|---|---|
| `/dashboard` | **Operator Operational Dashboard** | `dashboard` | Real-time operational signals, tasks, and status for Operator |
| `/entity-school` | **School Domain Workspace** | `entity_school` | Domain interface for School Domain Workspace |
| `/entity-instructor` | **Instructor Domain Workspace** | `entity_instructor` | Domain interface for Instructor Domain Workspace |
| `/entity-portal` | **Portal Domain Workspace** | `entity_portal` | Domain interface for Portal Domain Workspace |
| `/entity-exam` | **Exam Domain Workspace** | `entity_exam` | Domain interface for Exam Domain Workspace |
| `/entity-student` | **Student Domain Workspace** | `entity_student` | Domain interface for Student Domain Workspace |
| `/entity-enrollment` | **Enrollment Domain Workspace** | `entity_enrollment` | Domain interface for Enrollment Domain Workspace |
| `/profile` | **Operator Profile & Security** | `profile` | User credentials, contact details, and session management for Operator |

## 📐 Low-Level Design (LLD) Specifications & Reasoning Graphs

### [OPERATOR] School Domain Workspace (`/entity-school`)

- **Layout**: `master_detail_grid`
- **Composed Sub-Components**: `SchoolDomainInspector`, `SchoolAttributeMatrix`, `SchoolActivityTimeline`, `SchoolMetricsHeaderCard`, `SchoolActionDrawer`
- **Backing REST Endpoints**: `GET /api/schools`, `POST /api/schools`, `GET /api/schools/{id}`, `PUT /api/schools/{id}`
- **Validation Rules**: School identifier must be unique
- **Structured Reasoning Provenance**:
  - `(entity_school)` $\xrightarrow{\text{requires}}$ `(Domain Workspace & Inspector)`

**Tab & Form Field Breakdown**:

**Tab: School Overview**
- *Fields*: entity_school_identifier (string), title / label (string), operationalStatus (badge), createdDate (datetime), assignedActor (string)
- *User Actions*: Create School, Edit School, Archive School, Export CSV

### [OPERATOR] Instructor Domain Workspace (`/entity-instructor`)

- **Layout**: `master_detail_grid`
- **Composed Sub-Components**: `InstructorDomainInspector`, `InstructorAttributeMatrix`, `InstructorActivityTimeline`, `InstructorMetricsHeaderCard`, `InstructorActionDrawer`
- **Backing REST Endpoints**: `GET /api/instructors`, `POST /api/instructors`, `GET /api/instructors/{id}`, `PUT /api/instructors/{id}`
- **Validation Rules**: Instructor identifier must be unique
- **Structured Reasoning Provenance**:
  - `(entity_instructor)` $\xrightarrow{\text{requires}}$ `(Domain Workspace & Inspector)`

**Tab & Form Field Breakdown**:

**Tab: Instructor Overview**
- *Fields*: entity_instructor_identifier (string), title / label (string), operationalStatus (badge), createdDate (datetime), assignedActor (string)
- *User Actions*: Create Instructor, Edit Instructor, Archive Instructor, Export CSV

### [OPERATOR] Portal Domain Workspace (`/entity-portal`)

- **Layout**: `master_detail_grid`
- **Composed Sub-Components**: `PortalDomainInspector`, `PortalAttributeMatrix`, `PortalActivityTimeline`, `PortalMetricsHeaderCard`, `PortalActionDrawer`
- **Backing REST Endpoints**: `GET /api/portals`, `POST /api/portals`, `GET /api/portals/{id}`, `PUT /api/portals/{id}`
- **Validation Rules**: Portal identifier must be unique
- **Structured Reasoning Provenance**:
  - `(entity_portal)` $\xrightarrow{\text{requires}}$ `(Domain Workspace & Inspector)`

**Tab & Form Field Breakdown**:

**Tab: Portal Overview**
- *Fields*: entity_portal_identifier (string), title / label (string), operationalStatus (badge), createdDate (datetime), assignedActor (string)
- *User Actions*: Create Portal, Edit Portal, Archive Portal, Export CSV

### [OPERATOR] Exam Domain Workspace (`/entity-exam`)

- **Layout**: `master_detail_grid`
- **Composed Sub-Components**: `ExamDomainInspector`, `ExamAttributeMatrix`, `ExamActivityTimeline`, `ExamMetricsHeaderCard`, `ExamActionDrawer`
- **Backing REST Endpoints**: `GET /api/exams`, `POST /api/exams`, `GET /api/exams/{id}`, `PUT /api/exams/{id}`
- **Validation Rules**: Exam identifier must be unique
- **Structured Reasoning Provenance**:
  - `(entity_exam)` $\xrightarrow{\text{requires}}$ `(Domain Workspace & Inspector)`

**Tab & Form Field Breakdown**:

**Tab: Exam Overview**
- *Fields*: entity_exam_identifier (string), title / label (string), operationalStatus (badge), createdDate (datetime), assignedActor (string)
- *User Actions*: Create Exam, Edit Exam, Archive Exam, Export CSV

### [OPERATOR] Student Domain Workspace (`/entity-student`)

- **Layout**: `master_detail_grid`
- **Composed Sub-Components**: `StudentDomainInspector`, `StudentAttributeMatrix`, `StudentActivityTimeline`, `StudentMetricsHeaderCard`, `StudentActionDrawer`
- **Backing REST Endpoints**: `GET /api/students`, `POST /api/students`, `GET /api/students/{id}`, `PUT /api/students/{id}`
- **Validation Rules**: Student identifier must be unique
- **Structured Reasoning Provenance**:
  - `(entity_student)` $\xrightarrow{\text{requires}}$ `(Domain Workspace & Inspector)`

**Tab & Form Field Breakdown**:

**Tab: Student Overview**
- *Fields*: entity_student_identifier (string), title / label (string), operationalStatus (badge), createdDate (datetime), assignedActor (string)
- *User Actions*: Create Student, Edit Student, Archive Student, Export CSV

### [OPERATOR] Enrollment Domain Workspace (`/entity-enrollment`)

- **Layout**: `master_detail_grid`
- **Composed Sub-Components**: `EnrollmentDomainInspector`, `EnrollmentAttributeMatrix`, `EnrollmentActivityTimeline`, `EnrollmentMetricsHeaderCard`, `EnrollmentActionDrawer`
- **Backing REST Endpoints**: `GET /api/enrollments`, `POST /api/enrollments`, `GET /api/enrollments/{id}`, `PUT /api/enrollments/{id}`
- **Validation Rules**: Enrollment identifier must be unique
- **Structured Reasoning Provenance**:
  - `(entity_enrollment)` $\xrightarrow{\text{requires}}$ `(Domain Workspace & Inspector)`

**Tab & Form Field Breakdown**:

**Tab: Enrollment Overview**
- *Fields*: entity_enrollment_identifier (string), title / label (string), operationalStatus (badge), createdDate (datetime), assignedActor (string)
- *User Actions*: Create Enrollment, Edit Enrollment, Archive Enrollment, Export CSV

### [OPERATOR] Operator Profile & Security (`/profile`)

- **Layout**: `tabbed_card_layout`
- **Composed Sub-Components**: `AvatarUploader`, `BioHeaderCard`, `PersonalDetailsTab`, `SecurityPasswordModal`
- **Backing REST Endpoints**: `GET /api/account/profile`, `PUT /api/account/profile`, `PUT /api/auth/password`
- **Validation Rules**: Password must meet complexity requirements
- **Structured Reasoning Provenance**:
  - `(actor_operator)` $\xrightarrow{\text{requires}}$ `(Identity & Security)`

**Tab & Form Field Breakdown**:

**Tab: Personal Details**
- *Fields*: fullName (string), emailAddress (email), contactPhone (tel), roleDesignation (string, read-only)
- *User Actions*: Update Profile Info, Upload Avatar Image
...
- **REQ-PAGE-3**: Driving School Management View — Exam Tracking
  ...
  - _Inferred access level 'full_crud' from clause semantics_
  - _Source clause: 'Build driving school management portal with student enrollment, instructor scheduling, and exam trac'_
- **REQ-LLD-COMP-1**: [INSTRUCTOR] Instructor Dashboard — UI Component Hierarchy: MetricStatCardGrid, UpcomingEventsTimeline, QuickActionShortcuts, RecentActivityFeed
...
- **REQ-LLD-FIELDS-2**: [INSTRUCTOR] Instructor Dashboard (Overview) — Form Fields: kpiMetrics (object), announcements (array), pendingTasksCount (number), recentEvents (array) | Actions: Refresh Real-Time Metrics, Acknowledge Notification, Trigger Quick Action
...
- **REQ-LLD-API-3**: [INSTRUCTOR] Instructor Dashboard — Backing REST APIs: GET /api/dashboard/metrics, GET /api/dashboard/announcements, GET /api/notifications
...
- **REQ-LLD-COMP-4**: [INSTRUCTOR] Instructor Self-Profile — UI Component Hierarchy: AvatarUploader, BioHeaderCard, PersonalDetailsTab, OrganizationalCredentialsTab
...
- **REQ-LLD-FIELDS-5**: [INSTRUCTOR] Instructor Self-Profile (Personal Details) — Form Fields: fullName (string), personalEmail (email), mobileNumber (tel), dob (date), gender (select) | Actions: Update Personal Info, Upload Avatar Image
...
- **REQ-LLD-FIELDS-6**: [INSTRUCTOR] Instructor Self-Profile (Organizational Credentials) — Form Fields: identifierCode (string, read-only), departmentUnit (string, read-only), roleTitle (string, read-only), onboardingDate (date) | Actions: Download Digital ID Card
...
- **REQ-LLD-FIELDS-7**: [INSTRUCTOR] Instructor Self-Profile (Security & Authentication) — Form Fields: currentPassword (password), newPassword (password), confirmPassword (password), twoFactorToggle (boolean) | Actions: Change Password, Revoke Active Sessions
...
- **REQ-LLD-API-8**: [INSTRUCTOR] Instructor Self-Profile — Backing REST APIs: GET /api/profile, PUT /api/profile, POST /api/profile/avatar
  - _REST API contract for Instructor Self-Profile_
  - _Endpoints: GET /api/profile, PUT /api/profile, POST /api/profile/avatar_
- **REQ-LLD-COMP-9**: [INSTRUCTOR] Student Enrollment Workspace — UI Component Hierarchy: StudentEnrollmentDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
...
- **REQ-LLD-FIELDS-10**: [INSTRUCTOR] Student Enrollment Workspace (Student Enrollment Directory) — Form Fields: student_enrollmentId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create Student Enrollment, Edit Student Enrollment, Archive Student Enrollment
...
- **REQ-LLD-API-11**: [INSTRUCTOR] Student Enrollment Workspace — Backing REST APIs: GET /api/student_enrollments, POST /api/student_enrollments, GET /api/student_enrollments/{id}
...
- **REQ-LLD-COMP-12**: [INSTRUCTOR] Instructor Scheduling Workspace — UI Component Hierarchy: CalendarScheduleGrid, TimeSlotPickerMatrix, ResourceStaffSelector, BookingSummaryModal
...
- **REQ-LLD-FIELDS-13**: [INSTRUCTOR] Instructor Scheduling Workspace (Appointment Booking) — Form Fields: serviceTypeId (select), resourceStaffId (select), bookingDate (date), timeSlot (select), clientFullName (string) | Actions: Confirm Booking, Check Availability, Cancel Reservation
...
- **REQ-LLD-FIELDS-14**: [INSTRUCTOR] Instructor Scheduling Workspace (Reschedule & History) — Form Fields: bookingReference (string), originalDateTime (date), newDateTime (date), rescheduleReason (text), bookingStatus (badge) | Actions: Submit Reschedule Request, Download Receipt PDF
...
- **REQ-LLD-API-15**: [INSTRUCTOR] Instructor Scheduling Workspace — Backing REST APIs: GET /api/bookings/available-slots, POST /api/bookings, PATCH /api/bookings/{id}/reschedule
...
- **REQ-LLD-COMP-16**: [INSTRUCTOR] Exam Tracking Workspace — UI Component Hierarchy: ExamTrackingDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
...
- **REQ-LLD-FIELDS-17**: [INSTRUCTOR] Exam Tracking Workspace (Exam Tracking Directory) — Form Fields: exam_trackingId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create Exam Tracking, Edit Exam Tracking, Archive Exam Tracking
...
- **REQ-LLD-API-18**: [INSTRUCTOR] Exam Tracking Workspace — Backing REST APIs: GET /api/exam_trackings, POST /api/exam_trackings, GET /api/exam_trackings/{id}
...
- **REQ-LLD-COMP-19**: [INSTRUCTOR] Driving Workspace — UI Component Hierarchy: DrivingDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
...
- **REQ-LLD-FIELDS-20**: [INSTRUCTOR] Driving Workspace (Driving Directory) — Form Fields: drivingId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create Driving, Edit Driving, Archive Driving
...
- **REQ-LLD-API-21**: [INSTRUCTOR] Driving Workspace — Backing REST APIs: GET /api/drivings, POST /api/drivings, GET /api/drivings/{id}
  - _REST API contract for Driving Workspace_
  - _Endpoints: GET /api/drivings, POST /api/drivings, GET /api/drivings/{id}_
- **REQ-LLD-COMP-22**: [INSTRUCTOR] School Workspace — UI Component Hierarchy: SchoolDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
...
- **REQ-LLD-FIELDS-23**: [INSTRUCTOR] School Workspace (School Directory) — Form Fields: schoolId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create School, Edit School, Archive School
...
- **REQ-LLD-API-24**: [INSTRUCTOR] School Workspace — Backing REST APIs: GET /api/schools, POST /api/schools, GET /api/schools/{id}
  - _REST API contract for School Workspace_
  - _Endpoints: GET /api/schools, POST /api/schools, GET /api/schools/{id}_
- **REQ-LLD-COMP-25**: [INSTRUCTOR] Portal Workspace — UI Component Hierarchy: PortalDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
...
- **REQ-LLD-FIELDS-26**: [INSTRUCTOR] Portal Workspace (Portal Directory) — Form Fields: portalId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create Portal, Edit Portal, Archive Portal
...
- **REQ-LLD-API-27**: [INSTRUCTOR] Portal Workspace — Backing REST APIs: GET /api/portals, POST /api/portals, GET /api/portals/{id}
  - _REST API contract for Portal Workspace_
  - _Endpoints: GET /api/portals, POST /api/portals, GET /api/portals/{id}_
- **REQ-LLD-COMP-28**: [INSTRUCTOR] Enrollment Workspace — UI Component Hierarchy: EnrollmentDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
...
- **REQ-LLD-FIELDS-29**: [INSTRUCTOR] Enrollment Workspace (Enrollment Directory) — Form Fields: enrollmentId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create Enrollment, Edit Enrollment, Archive Enrollment
...
- **REQ-LLD-API-30**: [INSTRUCTOR] Enrollment Workspace — Backing REST APIs: GET /api/enrollments, POST /api/enrollments, GET /api/enrollments/{id}
  - _REST API contract for Enrollment Workspace_
  - _Endpoints: GET /api/enrollments, POST /api/enrollments, GET /api/enrollments/{id}_
- **REQ-LLD-COMP-31**: [INSTRUCTOR] Scheduling Workspace — UI Component Hierarchy: CalendarScheduleGrid, TimeSlotPickerMatrix, ResourceStaffSelector, BookingSummaryModal
  - _Canonical Low-Level Design expansion for Scheduling Workspace (/scheduling)_
  - _Layout type: calendar_dispatch_grid_
  - _Composed components: CalendarScheduleGrid, TimeSlotPickerMatrix, ResourceStaffSelector, BookingSummaryModal_
- **REQ-LLD-FIELDS-32**: [INSTRUCTOR] Scheduling Workspace (Appointment Booking) — Form Fields: serviceTypeId (select), resourceStaffId (select), bookingDate (date), timeSlot (select), clientFullName (string) | Actions: Confirm Booking, Check Availability, Cancel Reservation
  - _Mandatory field definitions for Scheduling Workspace -> Appointment Booking_
  - _Input fields: serviceTypeId (select), resourceStaffId (select), bookingDate (date), timeSlot (select), clientFullName (string)_
  - _Actions: Confirm Booking, Check Availability, Cancel Reservation_
- **REQ-LLD-FIELDS-33**: [INSTRUCTOR] Scheduling Workspace (Reschedule & History) — Form Fields: bookingReference (string), originalDateTime (date), newDateTime (date), rescheduleReason (text), bookingStatus (badge) | Actions: Submit Reschedule Request, Download Receipt PDF
  - _Mandatory field definitions for Scheduling Workspace -> Reschedule & History_
  - _Input fields: bookingReference (string), originalDateTime (date), newDateTime (date), rescheduleReason (text), bookingStatus (badge)_
  - _Actions: Submit Reschedule Request, Download Receipt PDF_
- **REQ-LLD-API-34**: [INSTRUCTOR] Scheduling Workspace — Backing REST APIs: GET /api/bookings/available-slots, POST /api/bookings, PATCH /api/bookings/{id}/reschedule
  - _REST API contract for Scheduling Workspace_
  - _Endpoints: GET /api/bookings/available-slots, POST /api/bookings, PATCH /api/bookings/{id}/reschedule_
- **REQ-LLD-COMP-35**: [INSTRUCTOR] Exam Workspace — UI Component Hierarchy: ExamDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
...
- **REQ-LLD-FIELDS-36**: [INSTRUCTOR] Exam Workspace (Exam Directory) — Form Fields: examId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create Exam, Edit Exam, Archive Exam
...
- **REQ-LLD-API-37**: [INSTRUCTOR] Exam Workspace — Backing REST APIs: GET /api/exams, POST /api/exams, GET /api/exams/{id}
  - _REST API contract for Exam Workspace_
  - _Endpoints: GET /api/exams, POST /api/exams, GET /api/exams/{id}_
- **REQ-LLD-COMP-38**: [INSTRUCTOR] Tracking Workspace — UI Component Hierarchy: TrackingDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
...
- **REQ-LLD-FIELDS-39**: [INSTRUCTOR] Tracking Workspace (Tracking Directory) — Form Fields: trackingId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create Tracking, Edit Tracking, Archive Tracking
...
- **REQ-LLD-API-40**: [INSTRUCTOR] Tracking Workspace — Backing REST APIs: GET /api/trackings, POST /api/trackings, GET /api/trackings/{id}
  - _REST API contract for Tracking Workspace_
  - _Endpoints: GET /api/trackings, POST /api/trackings, GET /api/trackings/{id}_
- **REQ-LLD-COMP-41**: [INSTRUCTOR] Student Workspace — UI Component Hierarchy: StudentDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
...
- **REQ-LLD-FIELDS-42**: [INSTRUCTOR] Student Workspace (Student Directory) — Form Fields: studentId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create Student, Edit Student, Archive Student
...
- **REQ-LLD-API-43**: [INSTRUCTOR] Student Workspace — Backing REST APIs: GET /api/students, POST /api/students, GET /api/students/{id}
  - _REST API contract for Student Workspace_
  - _Endpoints: GET /api/students, POST /api/students, GET /api/students/{id}_
- **REQ-LLD-COMP-44**: [INSTRUCTOR] Instructor Workspace — UI Component Hierarchy: InstructorDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
...
- **REQ-LLD-FIELDS-45**: [INSTRUCTOR] Instructor Workspace (Instructor Directory) — Form Fields: instructorId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create Instructor, Edit Instructor, Archive Instructor
...
- **REQ-LLD-API-46**: [INSTRUCTOR] Instructor Workspace — Backing REST APIs: GET /api/instructors, POST /api/instructors, GET /api/instructors/{id}
  - _REST API contract for Instructor Workspace_
  - _Endpoints: GET /api/instructors, POST /api/instructors, GET /api/instructors/{id}_
- **REQ-LLD-COMP-47**: [INSTRUCTOR] Instructor Document Vault — UI Component Hierarchy: FileUploadZone, VirusScanStatusPill, CategoryFolderTabs, DocumentPreviewModal
...
- **REQ-LLD-FIELDS-48**: [INSTRUCTOR] Instructor Document Vault (Document Vault) — Form Fields: fileName (string), fileSizeBytes (number), fileCategory (select), uploadedBy (string), securityScanStatus (badge) | Actions: Upload Document, Download Document, Delete Document
...
- **REQ-LLD-API-49**: [INSTRUCTOR] Instructor Document Vault — Backing REST APIs: GET /api/documents, POST /api/documents/upload, DELETE /api/documents/{id}
...
- **REQ-LLD-COMP-50**: [DRIVING SCHOOL MANAGEMENT] Driving School Management Dashboard — UI Component Hierarchy: MetricStatCardGrid, UpcomingEventsTimeline, QuickActionShortcuts, RecentActivityFeed
  - _Canonical Low-Level Design expansion for Driving School Management Dashboard (/dashboard)_
  - _Layout type: metrics_grid_
  - _Composed components: MetricStatCardGrid, UpcomingEventsTimeline, QuickActionShortcuts, RecentActivityFeed_
- **REQ-LLD-FIELDS-51**: [DRIVING SCHOOL MANAGEMENT] Driving School Management Dashboard (Overview) — Form Fields: kpiMetrics (object), announcements (array), pendingTasksCount (number), recentEvents (array) | Actions: Refresh Real-Time Metrics, Acknowledge Notification, Trigger Quick Action
  - _Mandatory field definitions for Driving School Management Dashboard -> Overview_
  - _Input fields: kpiMetrics (object), announcements (array), pendingTasksCount (number), recentEvents (array)_
  - _Actions: Refresh Real-Time Metrics, Acknowledge Notification, Trigger Quick Action_
- **REQ-LLD-API-52**: [DRIVING SCHOOL MANAGEMENT] Driving School Management Dashboard — Backing REST APIs: GET /api/dashboard/metrics, GET /api/dashboard/announcements, GET /api/notifications
  - _REST API contract for Driving School Management Dashboard_
  - _Endpoints: GET /api/dashboard/metrics, GET /api/dashboard/announcements, GET /api/notifications_
- **REQ-LLD-COMP-53**: [DRIVING SCHOOL MANAGEMENT] Driving School Management Self-Profile — UI Component Hierarchy: AvatarUploader, BioHeaderCard, PersonalDetailsTab, OrganizationalCredentialsTab
  - _Canonical Low-Level Design expansion for Driving School Management Self-Profile (/profile)_
  - _Layout type: tabbed_card_layout_
  - _Composed components: AvatarUploader, BioHeaderCard, PersonalDetailsTab, OrganizationalCredentialsTab_
- **REQ-LLD-FIELDS-54**: [DRIVING SCHOOL MANAGEMENT] Driving School Management Self-Profile (Personal Details) — Form Fields: fullName (string), personalEmail (email), mobileNumber (tel), dob (date), gender (select) | Actions: Update Personal Info, Upload Avatar Image
  - _Mandatory field definitions for Driving School Management Self-Profile -> Personal Details_
  - _Input fields: fullName (string), personalEmail (email), mobileNumber (tel), dob (date), gender (select)_
  - _Actions: Update Personal Info, Upload Avatar Image_
- **REQ-LLD-FIELDS-55**: [DRIVING SCHOOL MANAGEMENT] Driving School Management Self-Profile (Organizational Credentials) — Form Fields: identifierCode (string, read-only), departmentUnit (string, read-only), roleTitle (string, read-only), onboardingDate (date) | Actions: Download Digital ID Card
  - _Mandatory field definitions for Driving School Management Self-Profile -> Organizational Credentials_
  - _Input fields: identifierCode (string, read-only), departmentUnit (string, read-only), roleTitle (string, read-only), onboardingDate (date)_
  - _Actions: Download Digital ID Card_
- **REQ-LLD-FIELDS-56**: [DRIVING SCHOOL MANAGEMENT] Driving School Management Self-Profile (Security & Authentication) — Form Fields: currentPassword (password), newPassword (password), confirmPassword (password), twoFactorToggle (boolean) | Actions: Change Password, Revoke Active Sessions
  - _Mandatory field definitions for Driving School Management Self-Profile -> Security & Authentication_
  - _Input fields: currentPassword (password), newPassword (password), confirmPassword (password), twoFactorToggle (boolean)_
  - _Actions: Change Password, Revoke Active Sessions_
- **REQ-LLD-API-57**: [DRIVING SCHOOL MANAGEMENT] Driving School Management Self-Profile — Backing REST APIs: GET /api/profile, PUT /api/profile, POST /api/profile/avatar
  - _REST API contract for Driving School Management Self-Profile_
  - _Endpoints: GET /api/profile, PUT /api/profile, POST /api/profile/avatar_
- **REQ-LLD-COMP-58**: [DRIVING SCHOOL MANAGEMENT] Student Enrollment Workspace — UI Component Hierarchy: StudentEnrollmentDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
  - _Canonical Low-Level Design expansion for Student Enrollment Workspace (/student-enrollment)_
  - _Layout type: master_detail_grid_
  - _Composed components: StudentEnrollmentDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal_
- **REQ-LLD-FIELDS-59**: [DRIVING SCHOOL MANAGEMENT] Student Enrollment Workspace (Student Enrollment Directory) — Form Fields: student_enrollmentId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create Student Enrollment, Edit Student Enrollment, Archive Student Enrollment
  - _Mandatory field definitions for Student Enrollment Workspace -> Student Enrollment Directory_
  - _Input fields: student_enrollmentId (string), title / name (string), status (badge), categoryType (select), assignedTo (string)_
  - _Actions: Create Student Enrollment, Edit Student Enrollment, Archive Student Enrollment_
- **REQ-LLD-API-60**: [DRIVING SCHOOL MANAGEMENT] Student Enrollment Workspace — Backing REST APIs: GET /api/student_enrollments, POST /api/student_enrollments, GET /api/student_enrollments/{id}
  - _REST API contract for Student Enrollment Workspace_
  - _Endpoints: GET /api/student_enrollments, POST /api/student_enrollments, GET /api/student_enrollments/{id}_
- **REQ-LLD-COMP-61**: [DRIVING SCHOOL MANAGEMENT] Instructor Scheduling Workspace — UI Component Hierarchy: CalendarScheduleGrid, TimeSlotPickerMatrix, ResourceStaffSelector, BookingSummaryModal
  - _Canonical Low-Level Design expansion for Instructor Scheduling Workspace (/instructor-scheduling)_
  - _Layout type: calendar_dispatch_grid_
  - _Composed components: CalendarScheduleGrid, TimeSlotPickerMatrix, ResourceStaffSelector, BookingSummaryModal_
- **REQ-LLD-FIELDS-62**: [DRIVING SCHOOL MANAGEMENT] Instructor Scheduling Workspace (Appointment Booking) — Form Fields: serviceTypeId (select), resourceStaffId (select), bookingDate (date), timeSlot (select), clientFullName (string) | Actions: Confirm Booking, Check Availability, Cancel Reservation
  - _Mandatory field definitions for Instructor Scheduling Workspace -> Appointment Booking_
  - _Input fields: serviceTypeId (select), resourceStaffId (select), bookingDate (date), timeSlot (select), clientFullName (string)_
  - _Actions: Confirm Booking, Check Availability, Cancel Reservation_
- **REQ-LLD-FIELDS-63**: [DRIVING SCHOOL MANAGEMENT] Instructor Scheduling Workspace (Reschedule & History) — Form Fields: bookingReference (string), originalDateTime (date), newDateTime (date), rescheduleReason (text), bookingStatus (badge) | Actions: Submit Reschedule Request, Download Receipt PDF
  - _Mandatory field definitions for Instructor Scheduling Workspace -> Reschedule & History_
  - _Input fields: bookingReference (string), originalDateTime (date), newDateTime (date), rescheduleReason (text), bookingStatus (badge)_
  - _Actions: Submit Reschedule Request, Download Receipt PDF_
- **REQ-LLD-API-64**: [DRIVING SCHOOL MANAGEMENT] Instructor Scheduling Workspace — Backing REST APIs: GET /api/bookings/available-slots, POST /api/bookings, PATCH /api/bookings/{id}/reschedule
  - _REST API contract for Instructor Scheduling Workspace_
  - _Endpoints: GET /api/bookings/available-slots, POST /api/bookings, PATCH /api/bookings/{id}/reschedule_
- **REQ-LLD-COMP-65**: [DRIVING SCHOOL MANAGEMENT] Exam Tracking Workspace — UI Component Hierarchy: ExamTrackingDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
  - _Canonical Low-Level Design expansion for Exam Tracking Workspace (/exam-tracking)_
  - _Layout type: master_detail_grid_
  - _Composed components: ExamTrackingDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal_
- **REQ-LLD-FIELDS-66**: [DRIVING SCHOOL MANAGEMENT] Exam Tracking Workspace (Exam Tracking Directory) — Form Fields: exam_trackingId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create Exam Tracking, Edit Exam Tracking, Archive Exam Tracking
  - _Mandatory field definitions for Exam Tracking Workspace -> Exam Tracking Directory_
  - _Input fields: exam_trackingId (string), title / name (string), status (badge), categoryType (select), assignedTo (string)_
  - _Actions: Create Exam Tracking, Edit Exam Tracking, Archive Exam Tracking_
- **REQ-LLD-API-67**: [DRIVING SCHOOL MANAGEMENT] Exam Tracking Workspace — Backing REST APIs: GET /api/exam_trackings, POST /api/exam_trackings, GET /api/exam_trackings/{id}
  - _REST API contract for Exam Tracking Workspace_
  - _Endpoints: GET /api/exam_trackings, POST /api/exam_trackings, GET /api/exam_trackings/{id}_
- **REQ-LLD-COMP-68**: [DRIVING SCHOOL MANAGEMENT] Driving Workspace — UI Component Hierarchy: DrivingDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
  - _Canonical Low-Level Design expansion for Driving Workspace (/driving)_
  - _Layout type: master_detail_grid_
  - _Composed components: DrivingDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal_
- **REQ-LLD-FIELDS-69**: [DRIVING SCHOOL MANAGEMENT] Driving Workspace (Driving Directory) — Form Fields: drivingId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create Driving, Edit Driving, Archive Driving
  - _Mandatory field definitions for Driving Workspace -> Driving Directory_
  - _Input fields: drivingId (string), title / name (string), status (badge), categoryType (select), assignedTo (string)_
  - _Actions: Create Driving, Edit Driving, Archive Driving_
- **REQ-LLD-API-70**: [DRIVING SCHOOL MANAGEMENT] Driving Workspace — Backing REST APIs: GET /api/drivings, POST /api/drivings, GET /api/drivings/{id}
  - _REST API contract for Driving Workspace_
  - _Endpoints: GET /api/drivings, POST /api/drivings, GET /api/drivings/{id}_
- **REQ-LLD-COMP-71**: [DRIVING SCHOOL MANAGEMENT] School Workspace — UI Component Hierarchy: SchoolDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
  - _Canonical Low-Level Design expansion for School Workspace (/school)_
  - _Layout type: master_detail_grid_
  - _Composed components: SchoolDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal_
- **REQ-LLD-FIELDS-72**: [DRIVING SCHOOL MANAGEMENT] School Workspace (School Directory) — Form Fields: schoolId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create School, Edit School, Archive School
  - _Mandatory field definitions for School Workspace -> School Directory_
  - _Input fields: schoolId (string), title / name (string), status (badge), categoryType (select), assignedTo (string)_
  - _Actions: Create School, Edit School, Archive School_
- **REQ-LLD-API-73**: [DRIVING SCHOOL MANAGEMENT] School Workspace — Backing REST APIs: GET /api/schools, POST /api/schools, GET /api/schools/{id}
  - _REST API contract for School Workspace_
  - _Endpoints: GET /api/schools, POST /api/schools, GET /api/schools/{id}_
- **REQ-LLD-COMP-74**: [DRIVING SCHOOL MANAGEMENT] Portal Workspace — UI Component Hierarchy: PortalDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
  - _Canonical Low-Level Design expansion for Portal Workspace (/portal)_
  - _Layout type: master_detail_grid_
  - _Composed components: PortalDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal_
- **REQ-LLD-FIELDS-75**: [DRIVING SCHOOL MANAGEMENT] Portal Workspace (Portal Directory) — Form Fields: portalId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create Portal, Edit Portal, Archive Portal
  - _Mandatory field definitions for Portal Workspace -> Portal Directory_
  - _Input fields: portalId (string), title / name (string), status (badge), categoryType (select), assignedTo (string)_
  - _Actions: Create Portal, Edit Portal, Archive Portal_
- **REQ-LLD-API-76**: [DRIVING SCHOOL MANAGEMENT] Portal Workspace — Backing REST APIs: GET /api/portals, POST /api/portals, GET /api/portals/{id}
  - _REST API contract for Portal Workspace_
  - _Endpoints: GET /api/portals, POST /api/portals, GET /api/portals/{id}_
- **REQ-LLD-COMP-77**: [DRIVING SCHOOL MANAGEMENT] Enrollment Workspace — UI Component Hierarchy: EnrollmentDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
  - _Canonical Low-Level Design expansion for Enrollment Workspace (/enrollment)_
  - _Layout type: master_detail_grid_
  - _Composed components: EnrollmentDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal_
- **REQ-LLD-FIELDS-78**: [DRIVING SCHOOL MANAGEMENT] Enrollment Workspace (Enrollment Directory) — Form Fields: enrollmentId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create Enrollment, Edit Enrollment, Archive Enrollment
  - _Mandatory field definitions for Enrollment Workspace -> Enrollment Directory_
  - _Input fields: enrollmentId (string), title / name (string), status (badge), categoryType (select), assignedTo (string)_
  - _Actions: Create Enrollment, Edit Enrollment, Archive Enrollment_
- **REQ-LLD-API-79**: [DRIVING SCHOOL MANAGEMENT] Enrollment Workspace — Backing REST APIs: GET /api/enrollments, POST /api/enrollments, GET /api/enrollments/{id}
  - _REST API contract for Enrollment Workspace_
  - _Endpoints: GET /api/enrollments, POST /api/enrollments, GET /api/enrollments/{id}_
- **REQ-LLD-COMP-80**: [DRIVING SCHOOL MANAGEMENT] Scheduling Workspace — UI Component Hierarchy: CalendarScheduleGrid, TimeSlotPickerMatrix, ResourceStaffSelector, BookingSummaryModal
  - _Canonical Low-Level Design expansion for Scheduling Workspace (/scheduling)_
  - _Layout type: calendar_dispatch_grid_
  - _Composed components: CalendarScheduleGrid, TimeSlotPickerMatrix, ResourceStaffSelector, BookingSummaryModal_
- **REQ-LLD-FIELDS-81**: [DRIVING SCHOOL MANAGEMENT] Scheduling Workspace (Appointment Booking) — Form Fields: serviceTypeId (select), resourceStaffId (select), bookingDate (date), timeSlot (select), clientFullName (string) | Actions: Confirm Booking, Check Availability, Cancel Reservation
  - _Mandatory field definitions for Scheduling Workspace -> Appointment Booking_
  - _Input fields: serviceTypeId (select), resourceStaffId (select), bookingDate (date), timeSlot (select), clientFullName (string)_
  - _Actions: Confirm Booking, Check Availability, Cancel Reservation_
- **REQ-LLD-FIELDS-82**: [DRIVING SCHOOL MANAGEMENT] Scheduling Workspace (Reschedule & History) — Form Fields: bookingReference (string), originalDateTime (date), newDateTime (date), rescheduleReason (text), bookingStatus (badge) | Actions: Submit Reschedule Request, Download Receipt PDF
  - _Mandatory field definitions for Scheduling Workspace -> Reschedule & History_
  - _Input fields: bookingReference (string), originalDateTime (date), newDateTime (date), rescheduleReason (text), bookingStatus (badge)_
  - _Actions: Submit Reschedule Request, Download Receipt PDF_
- **REQ-LLD-API-83**: [DRIVING SCHOOL MANAGEMENT] Scheduling Workspace — Backing REST APIs: GET /api/bookings/available-slots, POST /api/bookings, PATCH /api/bookings/{id}/reschedule
  - _REST API contract for Scheduling Workspace_
  - _Endpoints: GET /api/bookings/available-slots, POST /api/bookings, PATCH /api/bookings/{id}/reschedule_
- **REQ-LLD-COMP-84**: [DRIVING SCHOOL MANAGEMENT] Exam Workspace — UI Component Hierarchy: ExamDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
  - _Canonical Low-Level Design expansion for Exam Workspace (/exam)_
  - _Layout type: master_detail_grid_
  - _Composed components: ExamDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal_
- **REQ-LLD-FIELDS-85**: [DRIVING SCHOOL MANAGEMENT] Exam Workspace (Exam Directory) — Form Fields: examId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create Exam, Edit Exam, Archive Exam
  - _Mandatory field definitions for Exam Workspace -> Exam Directory_
  - _Input fields: examId (string), title / name (string), status (badge), categoryType (select), assignedTo (string)_
  - _Actions: Create Exam, Edit Exam, Archive Exam_
- **REQ-LLD-API-86**: [DRIVING SCHOOL MANAGEMENT] Exam Workspace — Backing REST APIs: GET /api/exams, POST /api/exams, GET /api/exams/{id}
  - _REST API contract for Exam Workspace_
  - _Endpoints: GET /api/exams, POST /api/exams, GET /api/exams/{id}_
- **REQ-LLD-COMP-87**: [DRIVING SCHOOL MANAGEMENT] Tracking Workspace — UI Component Hierarchy: TrackingDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
  - _Canonical Low-Level Design expansion for Tracking Workspace (/tracking)_
  - _Layout type: master_detail_grid_
  - _Composed components: TrackingDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal_
- **REQ-LLD-FIELDS-88**: [DRIVING SCHOOL MANAGEMENT] Tracking Workspace (Tracking Directory) — Form Fields: trackingId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create Tracking, Edit Tracking, Archive Tracking
  - _Mandatory field definitions for Tracking Workspace -> Tracking Directory_
  - _Input fields: trackingId (string), title / name (string), status (badge), categoryType (select), assignedTo (string)_
  - _Actions: Create Tracking, Edit Tracking, Archive Tracking_
- **REQ-LLD-API-89**: [DRIVING SCHOOL MANAGEMENT] Tracking Workspace — Backing REST APIs: GET /api/trackings, POST /api/trackings, GET /api/trackings/{id}
  - _REST API contract for Tracking Workspace_
  - _Endpoints: GET /api/trackings, POST /api/trackings, GET /api/trackings/{id}_
- **REQ-LLD-COMP-90**: [DRIVING SCHOOL MANAGEMENT] Student Workspace — UI Component Hierarchy: StudentDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
  - _Canonical Low-Level Design expansion for Student Workspace (/student)_
  - _Layout type: master_detail_grid_
  - _Composed components: StudentDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal_
- **REQ-LLD-FIELDS-91**: [DRIVING SCHOOL MANAGEMENT] Student Workspace (Student Directory) — Form Fields: studentId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create Student, Edit Student, Archive Student
  - _Mandatory field definitions for Student Workspace -> Student Directory_
  - _Input fields: studentId (string), title / name (string), status (badge), categoryType (select), assignedTo (string)_
  - _Actions: Create Student, Edit Student, Archive Student_
- **REQ-LLD-API-92**: [DRIVING SCHOOL MANAGEMENT] Student Workspace — Backing REST APIs: GET /api/students, POST /api/students, GET /api/students/{id}
  - _REST API contract for Student Workspace_
  - _Endpoints: GET /api/students, POST /api/students, GET /api/students/{id}_
- **REQ-LLD-COMP-93**: [DRIVING SCHOOL MANAGEMENT] Instructor Workspace — UI Component Hierarchy: InstructorDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
  - _Canonical Low-Level Design expansion for Instructor Workspace (/instructor)_
  - _Layout type: master_detail_grid_
  - _Composed components: InstructorDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal_
- **REQ-LLD-FIELDS-94**: [DRIVING SCHOOL MANAGEMENT] Instructor Workspace (Instructor Directory) — Form Fields: instructorId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create Instructor, Edit Instructor, Archive Instructor
  - _Mandatory field definitions for Instructor Workspace -> Instructor Directory_
  - _Input fields: instructorId (string), title / name (string), status (badge), categoryType (select), assignedTo (string)_
  - _Actions: Create Instructor, Edit Instructor, Archive Instructor_
- **REQ-LLD-API-95**: [DRIVING SCHOOL MANAGEMENT] Instructor Workspace — Backing REST APIs: GET /api/instructors, POST /api/instructors, GET /api/instructors/{id}
  - _REST API contract for Instructor Workspace_
  - _Endpoints: GET /api/instructors, POST /api/instructors, GET /api/instructors/{id}_
- **REQ-LLD-COMP-96**: [DRIVING SCHOOL MANAGEMENT] Driving School Management Document Vault — UI Component Hierarchy: FileUploadZone, VirusScanStatusPill, CategoryFolderTabs, DocumentPreviewModal
  - _Canonical Low-Level Design expansion for Driving School Management Document Vault (/documents)_
  - _Layout type: document_vault_
  - _Composed components: FileUploadZone, VirusScanStatusPill, CategoryFolderTabs, DocumentPreviewModal_
- **REQ-LLD-FIELDS-97**: [DRIVING SCHOOL MANAGEMENT] Driving School Management Document Vault (Document Vault) — Form Fields: fileName (string), fileSizeBytes (number), fileCategory (select), uploadedBy (string), securityScanStatus (badge) | Actions: Upload Document, Download Document, Delete Document
  - _Mandatory field definitions for Driving School Management Document Vault -> Document Vault_
  - _Input fields: fileName (string), fileSizeBytes (number), fileCategory (select), uploadedBy (string), securityScanStatus (badge)_
  - _Actions: Upload Document, Download Document, Delete Document_
- **REQ-LLD-API-98**: [DRIVING SCHOOL MANAGEMENT] Driving School Management Document Vault — Backing REST APIs: GET /api/documents, POST /api/documents/upload, DELETE /api/documents/{id}
...
- **REQ-LLD-API-138**: [STUDENT] Tracking Workspace — Backing REST APIs: GET /api/trackings, POST /api/trackings, GET /api/trackings/{id}
  - _REST API contract for Tracking Workspace_
  - _Endpoints: GET /api/trackings, POST /api/trackings, GET /api/trackings/{id}_
- **REQ-LLD-COMP-139**: [STUDENT] Student Workspace — UI Component Hierarchy: StudentDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
  - _Canonical Low-Level Design expansion for Student Workspace (/student)_
  - _Layout type: master_detail_grid_
  - _Composed components: StudentDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal_
- **REQ-LLD-FIELDS-140**: [STUDENT] Student Workspace (Student Directory) — Form Fields: studentId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create Student, Edit Student, Archive Student
  - _Mandatory field definitions for Student Workspace -> Student Directory_
  - _Input fields: studentId (string), title / name (string), status (badge), categoryType (select), assignedTo (string)_
  - _Actions: Create Student, Edit Student, Archive Student_
- **REQ-LLD-API-141**: [STUDENT] Student Workspace — Backing REST APIs: GET /api/students, POST /api/students, GET /api/students/{id}
  - _REST API contract for Student Workspace_
  - _Endpoints: GET /api/students, POST /api/students, GET /api/students/{id}_
- **REQ-LLD-COMP-142**: [STUDENT] Instructor Workspace — UI Component Hierarchy: InstructorDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal
  - _Canonical Low-Level Design expansion for Instructor Workspace (/instructor)_
  - _Layout type: master_detail_grid_
  - _Composed components: InstructorDataGrid, SearchFilterBar, DetailInspectorDrawer, CreateEntityModal_
- **REQ-LLD-FIELDS-143**: [STUDENT] Instructor Workspace (Instructor Directory) — Form Fields: instructorId (string), title / name (string), status (badge), categoryType (select), assignedTo (string) | Actions: Create Instructor, Edit Instructor, Archive Instructor
  - _Mandatory field definitions for Instructor Workspace -> Instructor Directory_
  - _Input fields: instructorId (string), title / name (string), status (badge), categoryType (select), assignedTo (string)_
  - _Actions: Create Instructor, Edit Instructor, Archive Instructor_
- **REQ-LLD-API-144**: [STUDENT] Instructor Workspace — Backing REST APIs: GET /api/instructors, POST /api/instructors, GET /api/instructors/{id}
  - _REST API contract for Instructor Workspace_
  - _Endpoints: GET /api/instructors, POST /api/instructors, GET /api/instructors/{id}_

...
export async function GET() {
  try {
    ...
    let studentNodes: any[] = []
    if (student) {
      studentNodes = await db.studentRoadmapNode.findMany({
        where: { studentId: student.id }
      })
    }

    const mappedNodes = nodes.map(n => {
      const studentNode = studentNodes.find(sn => sn.nodeId === n.id)
      return {
        ...n,
        status: studentNode ? studentNode.status : 'AVAILABLE' // Default to AVAILABLE or LOCKED based on logic
      }
    })
